...
@Slf4j
public class ActionDeduceRules extends Action {
  ...
  public ActionDeduceRules() {
    super("deduce-rules");
    setDescription("Infer more generic rules from set of rules placed in file ");
...
/*
    // eliminate duplicates in rules from file (maybe assign weight = number of occurences)
    preprocessParameters();

    final Map<String, List<PatternMatch>> patternsByRelationTypes = groupByRelationTypes(patterns);

    for (final String relationType : patternsByRelationTypes.keySet()) {
      final List<PatternMatch> groupOfPatterns = patternsByRelationTypes.get(relationType);
      final List<PatternMatch> groupOfPatternsProcessed = new LinkedList<>();


      //zakładmy że kotwice NE już są maksymalnie uogólnione
      while (groupOfPatterns.size() > 0) {
        final PatternMatch startPattern = groupOfPatterns.get(0);

        final List<PatternMatch> oneKindOfPatterns = getAllPatternsAlike(startPattern);
        groupOfPatterns.remove(oneKindOfPatterns);
      }
