...
public class Database {
    ...
    public int registerDaemon(final String address) throws SQLException {
        final Statement statement = connection.createStatement();
        final ResultSet resultSet = statement.executeQuery(String.format("CALL register_daemon(\"%s\");", address));
        if (resultSet.next()) {
    ...
    public void unregisterDaemon(final int id) throws SQLException {
        final Statement statement = connection.createStatement();
        statement.executeQuery(String.format("CALL unregister_daemon(%d);", id));
    }

    public boolean checkRegistered(final int id) throws SQLException {
        final Statement statement = connection.createStatement();
    ...
    public void daemonNotReady(final int id) throws SQLException {
        final Statement statement = connection.createStatement();
    ...
    public void daemonReady(final int id) throws SQLException {
        final Statement statement = connection.createStatement();
        statement.executeQuery(String.format("CALL daemon_ready(%d);", id));
    }

    public Request getNextRequest(final int daemonId) throws SQLException {
    ...
    public void submitResult(final Request request) throws SQLException {
        try {
    ...
    public void submitError(final int requestId, final String msg) throws SQLException {
        final PreparedStatement preparedStatement = connection.prepareStatement(
                "CALL submit_error(?, ?);");
