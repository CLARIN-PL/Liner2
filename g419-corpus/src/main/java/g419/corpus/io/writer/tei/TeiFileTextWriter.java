...
public class TeiFileTextWriter extends TeiFileWriter {
  ...
  public TeiFileTextWriter(final OutputStream stream,
    ...
    writelnElement(Tei.TAG_TITLEPART,
        ImmutableMap.of("type", "title", "xml:id", "titlePart-1"), documentName);
    writelnEndElement();
    writelnEndElement();
  ...
  private void writeParagraph(final Paragraph paragraph) throws XMLStreamException {
    ...
    if (content.toString().trim().length() > 0) {
      writelnElement(Tei.TAG_PARAGRAPH,
          ImmutableMap.of("xml:id", paragraph.getId()), content.toString());
    }
  }
}
