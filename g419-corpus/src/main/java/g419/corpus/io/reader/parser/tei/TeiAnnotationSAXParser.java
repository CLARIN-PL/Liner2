...
public class TeiAnnotationSAXParser extends DefaultHandler implements HasLogger {
  ...
  final String group;
  final Optional<String> forcedType;
  ...
  public TeiAnnotationSAXParser(final String filename,
    ...
    this(filename, is, group, null, elements);
  }
  ...
  public TeiAnnotationSAXParser(final String filename,
    ...
    this.group = group;
    this.forcedType = Optional.ofNullable(forcedType);
  ...
  public void startElement(final String s, final String s1, final String elementName, final Attributes attributes) throws SAXException {
    ...
    switch (elementName.toLowerCase()) {
      ...
      case Tei.TAG_SEGMENT:
        ...
        forcedType.ifPresent(t -> currentGroup.setType(t));
  ...
  private List<Annotation> createAnnotations() {
    ...
    for (final SentenceGroup group : groups) {
      ...
      if (tokens.size() == 0) {
        getLogger().error("Annotation was ignored (id={}, file={}). The tokens not defined(no ptr elements)",
            group.getGroupId(), filename);
      } else if (group.getType() == null) {
        getLogger().error("Annotation was ignored (id={}, file={}). Unknown type",
            group.getGroupId(), filename);
      } else {
