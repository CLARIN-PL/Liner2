...
//				els-
    ...
    public static boolean hasRelation(Set<Edge> edges, String relationName) throws MaltChainedException {
      for (Edge e : edges) {
        for (SymbolTable t : e.getLabelTypes()) {
          if (LABEL_TABLE_NAME.equals(t.getName()) && relationName.equals(e.getLabelSymbol(t))) {
            return true;
          }
        }
      }
  ...
  public enum Gender {
    ...
    boolean equals(Gender g2, boolean masculinumTolerance, boolean undefinedIsAlwaysEqual) {
      if (undefinedIsAlwaysEqual && (UNDEFINED.equals(this) || UNDEFINED.equals(g2))) {
        return true;
      }
      if (masculinumTolerance && isMasculinum()) {
        return g2.isMasculinum();
      }
...
//			}
    ...
    private boolean attributeLookup(int radius, String attributeName, String attributeValue) {
      ...
      for (int i = start; i <= end; i++) {
        if (attributeValue.equalsIgnoreCase(this.ai.getAttributeValue(this.sentence.getTokens().get(i), attributeName))) {
          return true;
        }
      }
      ...
      if ("pri".equalsIgnoreCase(this.person)) {
        try {
          if ("ja".equalsIgnoreCase(this.ai.getAttributeValue(this.sentence.getTokens().get(this.positionInSentence - 2), "base"))
              && "nie".equalsIgnoreCase(this.ai.getAttributeValue(this.sentence.getTokens().get(this.positionInSentence - 1), "base"))) {
            return true;
          }
        ...
        try {
          if ("ja".equalsIgnoreCase(this.ai.getAttributeValue(this.sentence.getTokens().get(this.positionInSentence - 1), "base"))) {
            return true;
          }
        } catch (ArrayIndexOutOfBoundsException ex) {
        }
        ...
        try {
          if ("ty".equalsIgnoreCase(this.ai.getAttributeValue(this.sentence.getTokens().get(this.positionInSentence - 1), "base"))) {
            return true;
          }
        } catch (ArrayIndexOutOfBoundsException ex) {
        }
    ...
    public boolean hasMaltSubjectPriSec() throws MaltChainedException {
      ...
      for (MinosNoun subject : subjects) {
        if (this.person.equalsIgnoreCase(subject.person)) {
          return true;
        }
      }
    ...
    public boolean isNonAnaphoricVerbPriSec() throws MaltChainedException {
      ...
      if (preceedingPersonPriSec()) {
        return true;
      }
      if (hasMaltSubjectPriSec()) {
        return true;
      }
      return false;
    }
    ...
    public boolean isNonAnaphoricVerb() throws MaltChainedException {
      ...
      if (!PartsOfSpeech.contains(this.partOfSpeech)) {
        return true;
      }
      if (SETTINGS_PRECEEDING_PREDICATE && preceedingPredicate(predicateRadius)) {
        return true;
      }
      if (SETTINGS_MOOSE && mooseCriterion()) {
        return true;
      }
      if (SETTINGS_AUX_IT && auxiliaryIt()) {
        return true;
      }
      if (SETTINGS_ADV_BETTER_WORSE && advBetterWorse()) {
        return true;
      }
      if (SETTINGS_TAKSIE && taksie()) {
        return true;
      }

      if (SETTINGS_NEUTER_ADV && neuterAdv()) {
        return true;
      }
      if (SETTINGS_NEUTER_INT && neuterInt()) {
        return true;
      }
      if (SETTINGS_MAYBE && maybe()) {
        return true;
      }
    ...
    public boolean hasSubject(Document document) throws MaltChainedException {
      if (hasMaltSubject(SETTINGS_CHECK_MALT_SUBJ_AGREEMENT)) {
        return true;
      }
      if (hasChunkrelSubject(document)) {
        return true;
      }
      if (hasContextSubject()) {
        return true;
      }
    ...
    public boolean isZeroAnaphora(Document document) throws MaltChainedException {
      if (isPriSec()) {
        if (isNonAnaphoricVerbPriSec()) {
          return false;
        }
      } else {
        if (isNonAnaphoricVerb()) {
          return false;
        }
        if (hasSubject(document)) {
          return false;
        }
      }
    ...
    public boolean hasMaltSubject(boolean checkSubjectAgreement) throws MaltChainedException {
      ...
      if (maltSubjects == null || maltSubjects.size() <= 0) {
        return false;
      }
      if (!checkSubjectAgreement) {
        return true;
      }

      for (MinosNoun subject : maltSubjects) {
        if (subject.checkAgreement(this)) {
          return true;
        }
      }
    ...
    public boolean hasChunkrelSubject(Document document) {
      List<Annotation> chunkVP = this.sentence.getChunksAt(sentence.getTokens().indexOf(this.verb), Arrays.asList(Pattern.compile("chunk_vp")));
      if (chunkVP.isEmpty()) {
        return false;
      }
...
//			TokenAttributeIndex ai = sentence.getAttributeIndex();
    ...
    public boolean checkAgreement(MinosVerb verb) {
      if (isNeuterNumeral(verb)) {
        return true;
      }
      if (nieMaGen(verb)) {
        return true;
      }
      if (this.fromChunkrel) {
        return true;
      }
